...
class RegistrPage(BasePage):
    ...
    def __init__(self, browser, url, timeout=10):
        ...
        self.first_name = self.wait.until(EC.presence_of_element_located(RegistrPageLocators.FIRST_NAME))
        self.last_name = self.wait.until(EC.presence_of_element_located(RegistrPageLocators.LAST_NAME))
        self.address = self.wait.until(EC.presence_of_element_located(RegistrPageLocators.ADDRESS))
        self.pwd = self.wait.until(EC.presence_of_element_located(RegistrPageLocators.PASSWORD))
        self.pwd_conf = self.wait.until(EC.presence_of_element_located(RegistrPageLocators.PASSWORD_CONFIRM))
    ...
    def should_be_logo_on_header(self):
        ...
        assert self.is_element_present(*RegistrPageLocators.LOGO_HEADER)
    ...
    def should_be_registr_form(self):
        ...
        sp = []
        right = self.wait.until(EC.presence_of_all_elements_located(RegistrPageLocators.RIGHT_SIDE))
    ...
    def should_be_logo(self):
        ...
        sp = []
        left = self.wait.until(EC.presence_of_all_elements_located(RegistrPageLocators.LEFT_SIDE))
    ...
    def should_be_default_region(self, city):
        ...
        default_region = self.wait.until(EC.presence_of_element_located(RegistrPageLocators.REGION))
    ...
    def should_be_select_some_region(self, city):
        ...
        how, what = RegistrPageLocators.SELECT_REG2
        what = what.replace('$', city)
        new_locator = (how, what)
        self.wait.until(EC.presence_of_element_located(RegistrPageLocators.SELECT_REG1)).click()
        self.wait.until(EC.presence_of_element_located(new_locator)).click()
        region = self.wait.until(EC.presence_of_element_located(RegistrPageLocators.REGION))
    ...
    def choosing_registration_option(self, address):
        ...
        assert self.is_element_present(*RegistrPageLocators.REGISTER_CONF_FORM)

    def send_all_fields_is_empty(self):
        for el in [self.first_name, self.last_name, self.address, self.pwd, self.pwd_conf]:
            el.send_keys('')
        self.wait.until(EC.presence_of_element_located(
            RegistrPageLocators.REGISTER_BUTTON)).click()
        assert self.is_element_present(*RegistrPageLocators.TEXT_ERROR)
    ...
    def params_password_length(self, type_pass: str, param: int | str, exp=True):
        ...
        if isinstance(param, int):
            ...
            user_data_val = param
        self.change_user_data(user_data_key, user_data_val)
        error_text = self.wait.until(EC.presence_of_element_located(RegistrPageLocators.COUNT_SYMB_ERROR))
    ...
    def valid_passwords_equal(self, exp=True):
        ...
        if exp:
            ...
            assert self.is_element_present(*RegistrPageLocators.COUNT_SYMB_ERROR)
            assert self.is_element_present(*RegistrPageLocators.REGISTER_FORM)
