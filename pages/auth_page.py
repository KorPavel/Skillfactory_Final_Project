...
from settings import valid_user_data, get_nums, letters_en
...
class AuthPage(BasePage):
    ...
    def click_tab_phone(self):
        ...
        phone_tab.click()
        assert 'active' in phone_tab.get_attribute('class')
    ...
    def click_tab_ls(self):
        ...
        ls_tab.click()
        assert 'active' in ls_tab.get_attribute('class')

    def insert_user_data_to_fields(self, login, password):
        self.browser.find_element(*AuthPageLocators.LOGIN_AREA).send_keys(login)
        self.browser.find_element(*AuthPageLocators.PASSWORD_EYE).click()  # Открыть пароль
        self.browser.find_element(*AuthPageLocators.PASSWORD_AREA).send_keys(password)
    ...
    def send_params_to_relevant_areas(self, login, pwd):
        ...
        login_area = self.wait.until(EC.presence_of_element_located(AuthPageLocators.LOGIN_AREA))
        login_area.send_keys(login)
        pass_area = self.wait.until(EC.presence_of_element_located(AuthPageLocators.PASSWORD_AREA))
